...
<?php
    ...
    while ($row = pg_fetch_array($result)) {
        ...
        echo '<tr onclick="toggledesc(this)"><td><a href="/pie/'.$row['id'].'" target="_blank">'.$row['name'].'</a></td><td>'.$row['num'].'</td><td><meter value="'.$state.'" min="0" max="100" low="33" high="67">'.$state.'&nbsp;%</meter></td><td><a href="http://www.openstreetmap.org/user/'.$row['nick'].'" target="_blank">'.$row['nick'].'</a></td><td>'.$created.'</td><td>'.($row['ends'] ? $row['ends'] : '—').'</td></tr>';
